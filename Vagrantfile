...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  if(!File.exist?(STORM_ARCHIVE))
    `wget -N https://dl.dropboxusercontent.com/s/p5wf0hsdab5n9kn/storm-0.9.0-rc2.zip`
  end
  ...
  config.vm.define "nimbus" do |nimbus|
    ...
    nimbus.vm.provision "shell", path: "install-storm.sh"
  ...
  config.vm.define "supervisor" do |supervisor|
    ...
    supervisor.vm.provision "shell", path: "install-storm.sh"
